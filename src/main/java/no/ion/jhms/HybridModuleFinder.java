...
class HybridModuleFinder implements AutoCloseable {
    ...
    private void readModularJar(Path path, boolean ignoreNonModularJar) {
        // There's also a Path::endsWith, but that creates a path of the last argument and much more.
        ...
        // ModuleFinder.of() can be used to search for 1. a modular jar, 2. modular jars in directory, 3. exploded
        // modular jars in directory. It's an error if the same module (name) is found more than once. Therefore,
        // the only way to use ModuleFinder.of() in JHMS is (1).
        ...
        if (duplicateJar != null) {
            ...
            // It might be interesting to support the following:
            //  1. By default, we'll find the proper and strict versions of the hybrid modules.
            //  2. A hybrid module with a security patch could be put on the module path effectively
            //     overriding the original. Thus 2 name/version modular jars may be reachable, but only
            //     one should be used.
