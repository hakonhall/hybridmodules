...
public class HybridModuleResolverTest {
    ...
    public void test() throws ClassNotFoundException {
        try (var finder = HybridModuleFinder.of(Paths.get("src", "test", "resources"))) {
            ...
            assertContains(reads, "no.ion.jhms.test.FindHybridModule.two.exported", "find.hybrid.module.two@1.2.3");
            assertContains(reads, "no.ion.jhms.test.FindHybridModule", "find.hybrid.module.two@1.2.3");
            assertContains(reads, "no.ion.jhms.test.FindHybridModule.one.exported", "find.hybrid.module.one@1.2.3");
            assertEquals(3, reads.size());

            HybridModuleClassLoader classLoader = hybridModule.getClassLoader();
            Class<?> c1 = classLoader.loadExportedClass("no.ion.jhms.test.FindHybridModule.two.exported.TwoExportedPublic");
        }
    }
